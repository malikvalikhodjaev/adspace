...
The public catalogue has 63 screen-reference cards with 63 distinct local photographs. They sit in the existing screen grid, not in a separate promotional section. The homepage now shows a restrained four-photo mosaic sourced from four different catalogues. The reference cards come from five operator catalogues:
...
Catalogue cards and homepage tiles use 960×540 WebP previews prepared with `npm run photos:previews` and a Lanczos resampling kernel. The detail view continues to use the original photographs. The previews are visual derivatives only; no source PDF or original photo is overwritten. Homepage labels name each photograph's source catalogue, not a Maydonlar partnership.
